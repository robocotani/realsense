...
# 領域の座標取得
def analysis_blob_flag(binary_img):
    # 2値画像のラベリング処理
...
# 赤色検出
mask_flag = flag_detect(img)
#ノイズ除去
mask_flag_new = removenoise(mask_flag)
# マスク画像を解析
x,y,w,h,area,center,cnt = analysis_blob_flag(mask_flag_new)
...
if cnt==0: #フラッグ未検出
 ...
 # 結果表示
cv2.imshow("flag/Frame_flag_3.5m.jpg", img)
cv2.imshow("flag/Mask_flag_3.5m.jpg", mask_flag_new)
